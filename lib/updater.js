...
var Updater = function(options) {
    ...
    var notifier = options.notifier,
        ...
        notifyReadyPlot = function(farmer, plot, callback) {
            notifier.notify(farmer, "Your plot is ready", "readyplot", {plot: plot}, callback); 
        },
        updateEmptyPlot = function(plot, callback) {
            var now = Date.now();
            log.info("Checking empty plot ");
            if ( plot.state == Plot.READY || plot.state == Plot.RAW ) {
            ...
            } else if (plot.state == Plot.PLOWING) {
               // Check plowing...
               log.info("Plot is plowing with a " + plot.equipment);
               if ( now >= plot.plowcompleted ) {
                   async.waterfall([
                       function(callback) {
                           Farmer.get(plot.owner, callback);
                       },
                       function(farmer, callback) {
                           notifyReadyPlot(farmer, plot, callback);
                       },
                       function(callback) {
                           plot.state = Plot.READY;
                           plot.plowcompleted = null;
                           plot.save(callback);
                       }
                   ],callback);
               }
